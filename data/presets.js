const PRESETS = {
    ...
    'Max Efficiency': {
        ...
        builds: [
            ...
            { char: 'Fran', jobs: ['Monk', 'Time Battlemage'], espers: ['Ultima', 'Zeromus', 'Zodiark'], role: 'Support Mage', why: 'Critical Trinity Node: Swiftness 3, Channeling 3, Renew access, and natural Hastega (Time Battlemage). Ultimate support powerhouse.', gambits: ['Ally: Status ≠ Haste → Hastega', 'Ally: Any → Bravery', 'Ally: HP Critical → Renew', 'Foe: HP ≥ 10000 → Expose', 'Foe: Nearest Visible → Attack'], gear: ['Kanya | Best pole (109 ATK, Holy element, full 1.25x Genji bonus)', 'Grand Armor | Heavy Armor 12 for STR scaling', 'Golden Amulet | LP +10% boost', 'Ribbon | Status immunity'] },
    ...
    'Leader Trinity': {
        ...
        builds: [
            ...
            { char: 'Fran', jobs: ['Monk', 'Time Battlemage'], espers: ['Ultima', 'Zeromus', 'Zodiark'], role: 'Support', why: 'Buff and debuff specialist with natural Hastega (Time Battlemage). Primary source of Expose and Renew. Ultima unlocks Monk Swiftness x2, Zeromus unlocks Monk Channeling (infinite MP), Zodiark unlocks Monk Renew. Complete utility powerhouse with Hastega + Renew + Swiftness + Channeling.', gambits: ['Ally: Status ≠ Haste → Hastega', 'Ally: Any → Bravery', 'Ally: HP Critical → Renew', 'Foe: HP ≥ 10000 → Expose', 'Foe: Nearest Visible → Attack'], gear: ['Kanya | Best pole (109 ATK, Holy element, full 1.25x Genji bonus)', 'Grand Armor | Maximum physical defense for STR scaling', 'Ribbon | Status immunity for support role', 'Golden Amulet | LP boost for faster progression'] },
    ...
    'Yiazmat Specialist': {
        ...
        builds: [
            ...
            { char: 'Fran', jobs: ['Monk', 'Time Battlemage'], espers: ['Ultima', 'Zeromus'], role: 'Support', why: 'CRITICAL TRINITY NODE: Swiftness 3 (Ultima) + Channeling 3 (Zeromus) + Hastega (natural Time Battlemage). Provides Expose (Monk), Hastega, and infinite MP regeneration. Whale Whisker backup DPS (Yiazmat absorbs Holy so avoid Kanya).', gambits: ['Ally: Status ≠ Haste → Hastega', 'Foe: Yiazmat → Expose', 'Ally: Any → Bravery', 'Self: HP < 90% → Bubble', 'Foe: Nearest Visible → Attack'], gear: ['Whale Whisker | Non-elemental pole (Yiazmat absorbs Holy)', 'Grand Armor | Maximum physical defense for STR scaling', 'Genji Gloves | Combo boost for poles', 'Ribbon | Status immunity for long fight', 'Bubble Belt | HP threshold survival'] },
            ...
            { char: 'Balthier', jobs: ['Monk', 'Foebreaker'], espers: ['Zodiark', 'Hashmal'], role: 'Break Specialist', why: 'Full Break suite (Expose, Shear, Wither, Addle) maximizes party DPS. Zodiark provides Renew (Monk) for backup healing. Hashmal unlocks Foebreaker Curaga for self-sustaining during relief rotations. Whale Whisker provides strong non-elemental DPS (Yiazmat absorbs Holy so avoid Kanya).', gambits: ['Foe: Yiazmat → Expose', 'Foe: Status ≠ Shattered → Shear', 'Foe: Status ≠ Wither → Wither', 'Foe: Status ≠ Addle → Addle', 'Self: HP < 90% → Bubble', 'Ally: HP < 50% → Curaga'], gear: ['Whale Whisker | Non-elemental pole (Yiazmat absorbs Holy)', 'Grand Armor | Maximum physical defense', 'Genji Gloves | Combo boost for poles', 'Ribbon | Status immunity essential', 'Bubble Belt | HP threshold survival'] },
    ...
    'Spare No Expense': {
        ...
        requirements: {
            ...
            keyEspers: ['Ultima/Zeromus (Monk Swiftness+Channeling)', 'Chaos/Zodiark/Cuchulainn (Monk Utility Suite)', 'Shemhazai (Archer Heavy Armor)', 'Adrammelech/Famfrit (Uhlan Battle Lore+Sustain)'],
            recommendedLevel: 'Level 50+',
        ...
        builds: [
            ...
            { char: 'Fran', jobs: ['Monk', 'Time Battlemage'], espers: ['Ultima', 'Zeromus', 'Chaos', 'Zodiark', 'Cuchulainn'], role: 'Support Mage', why: 'Complete Monk Trinity + Time Battlemage synergy. Ultima (Swiftness x2 for Monk), Zeromus (Channeling for Monk), Chaos (Esunaga/Protectga/Shellga/Holy for Monk), Zodiark (Renew for Monk), Cuchulainn (Remedy Lore 3 for Monk). Time Battlemage provides natural Hastega (no esper needed). The ultimate buff/debuff/healing engine with Hastega, Haste, Bravery, Faith, Expose, Renew, Protectga, Shellga, Holy. Kanya with Genji Gloves for 1.25x combo-boosted Holy DPS when not supporting.', gambits: ['Ally: Status ≠ Haste → Hastega', 'Ally: Any → Bravery', 'Ally: Any → Faith', 'Ally: HP Critical → Renew', 'Foe: HP ≥ 10000 → Expose', 'Foe: Nearest Visible → Attack'], gear: ['Kanya | Best pole (109 ATK, Holy element, full Genji bonus)', 'Grand Armor | STR scaling for pole damage', 'Genji Gloves | PRIORITY 2: 1.25x combo multiplier for Kanya', 'Ribbon | Trial Mode status immunity', 'Golden Amulet | LP boost for utility unlocks'] },
            { char: 'Vaan', jobs: ['Bushi', 'Knight'], espers: ['Mateus', 'Hashmal', 'Belias'], role: 'Warrior', why: 'Wiki-recommended best Bushi pairing. Excalibur + White Robes synergy (50% Holy boost) or Masamune katana with MAG scaling. Mateus unlocks Knight healing (Curaga/Esuna/Cleanse/Regen), Hashmal adds upgraded healing suite (Curaja/Bravery/Faith), Belias provides Libra for enemy info. Can flex between Holy damage (Knight) and katana Berserk (Bushi) with self-sustain.', gambits: ['Self: HP < 90% → Bubble', 'Ally: HP < 50% → Curaga', 'Ally: Any → Bravery', 'Foe: Weak Holy → Attack (Excalibur)', 'Ally: Any → Berserk'], gear: ['Masamune | MAG-scaling katana for Berserk DPS', 'Excalibur | Holy sword with White Robes (50% boost)', 'White Robes | 50% Holy damage boost', 'Grand Armor | Heavy armor for frontline', 'Genji Gloves | PRIORITY 1: 1.8x combo rate for katanas', 'Ribbon | Trial Mode farmed status immunity'] },
            { char: 'Balthier', jobs: ['Archer', 'Foebreaker'], espers: ['Shemhazai'], role: 'Ranged DPS', why: 'Archer primary with Foebreaker augments. Shemhazai unlocks Archer Heavy Armor 10-12 (MANDATORY). Foebreaker provides maximum Battle Lores for bow damage scaling plus break utility (Expose/Shear before unloading arrows). Berserked bow DPS with Burning Bow for fire damage or Perseus Bow for raw power.', gambits: ['Foe: HP ≥ 10000 → Expose', 'Foe: HP ≥ 10000 → Shear', 'Self: → Berserk', 'Foe: Nearest Visible → Attack'], gear: ['Perseus Bow / Dhanusha | Top-tier endgame bows', 'Burning Bow | Fire element for sustained physical damage', 'Grand Armor | Heavy armor from Shemhazai', 'Germinas Boots | +50 Speed essential for bow damage', 'Ribbon | Trial Mode status immunity', 'Bubble Belt | HP survival for Berserked DPS'] },
            { char: 'Basch', jobs: ['Uhlan', 'Bushi'], espers: ['Adrammelech', 'Famfrit'], role: 'Spear DPS', why: 'Uhlan primary with Bushi augments. Holy Lance + White Robes = 50% Holy damage boost (same synergy as Excalibur). Zodiac Spear for non-Holy-weak enemies. Bushi provides Swiftness 3 for faster actions and Battle Lores for damage scaling. Adrammelech gives Uhlan Battle Lore. Famfrit provides Uhlan Potion Lore 3 for sustain.', gambits: ['Foe: Weak Holy → Attack (Holy Lance)', 'Self: → Berserk', 'Foe: Nearest Visible → Attack'], gear: ['Holy Lance | Holy spear with White Robes (50% boost)', 'Zodiac Spear | Best spear for non-Holy-weak (farm multiple)', 'White Robes | 50% Holy damage boost for Holy Lance', 'Grand Armor | Heavy armor alternative', 'Ribbon | Trial Mode status immunity', 'Bubble Belt | HP survival for Berserked DPS'] },
